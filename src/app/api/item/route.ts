...
export async function POST(req: Request) {
  ...
  const { vendorId, itemId, name, categoryId, priceCents, variants } = await req.json();
  ...
  // and typicalPriceCents becomes the "from" (min) price.
  type VIn = { label?: unknown; priceCents?: unknown; note?: unknown };
  let clean: { label: string; priceCents: number; note: string | null; sortOrder: number }[] | null = null;
  if (Array.isArray(variants)) {
    clean = (variants as VIn[])
      ...
      .map((v, i) => ({ label: String(v.label).trim(), priceCents: Math.round(v.priceCents as number), note: v.note ? String(v.note).trim() : null, sortOrder: i }));
  }
